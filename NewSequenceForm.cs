...
namespace LightSequencer
{
	public partial class NewSequenceForm : Form
	{
		...
		public NewSequenceForm(Sequence s)
		{
            ...
            HardwareHandler.HardwareChanged += new EventHandler(PhidgetHandler_HardwareChanged);

			//if(PhidgetHandler.IFKits.Count == 0)
            if(HardwareHandler.numOfChannels() == 0)
			{
				btnOK.Enabled = false;
        ...
        void PhidgetHandler_HardwareChanged(object sender, EventArgs e)
        {
            FillPhidgetGrid();
        }
		...
		private void FillPhidgetGrid()
		{
            ...
            for (int ch = 0; ch < HardwareHandler.numOfChannels(); ch++)
            {
                //int currentChannel = _numChannels;
                int currentChannel = ch;
                ...
                //{
                    ...
                    //dgvDevices.Rows.Add("Name", "1", ch, -1, ch);
                    dgvDevices.Rows.Add("Channel Name", 1, -1, ch); //TODO change this to reference channels[] lightchannel.physicalChannel data

                    //if (_edit && i < _sequence.Channels.Count)
                    if (_edit && 1 < _sequence.Channels.Count)
                    {
                        if (_sequence.MusicType == MusicType.MIDI)
                            //dgvDevices.Rows[i].Cells["colMIDIChannel"].Value = _sequence.Channels[i].MIDIChannel;
                            dgvDevices.Rows[1].Cells["colMIDIChannel"].Value = _sequence.Channels[1].MIDIChannel;

                        //dgvDevices.Rows[i].Cells["colPhidgetOutput"].Value = _sequence.Channels[i].OutputIndex;
                        dgvDevices.Rows[1].Cells["colPhidgetOutput"].Value = _sequence.Channels[1].OutputIndex;
                    }
                    else
                    {
                        ...
                        if (_sequence != null && _sequence.MusicType == MusicType.MIDI && _MIDIChannels != null && 1 < _MIDIChannels.Count)
                            //dgvDevices.Rows[i].Cells["colMIDIChannel"].Value = i;
                            dgvDevices.Rows[1].Cells["colMIDIChannel"].Value = 1;
                    }
                //}
            }
		...
		private void NewSequenceForm_FormClosed(object sender, FormClosedEventArgs e)
		{
            ...
            HardwareHandler.HardwareChanged -= PhidgetHandler_HardwareChanged;
		}
