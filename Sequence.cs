		...
		public void LoadV1Sequence(XmlDocument xmlDoc)
		{
			...
			for(int i = 0; i < numChannels; i++)
			{
				...
				if(node != null)
				{
                    ...
                    this.Channels.Add(new Channel(i, -3, int.Parse(node.Attributes["outputIndex"].Value), data.Length));
					for(int j = 0; j < data.Length; j++)
						this.Channels[i].Data[j] = (data[j] == '1');
				}
				else
					//this.Channels.Add(new Channel(i, -1, -1));
                    this.Channels.Add(new Channel(i, -3, -1));
			}
		}
		...
		public void LoadSequence(XmlDocument xmlDoc)
		{
			...
			for(int i = 0; i < numChannels; i++)
			{
				...
				if(node != null)
				{
                    ...
                    this.Channels.Add(new Channel(i, -4, int.Parse(node.Attributes["outputIndex"].Value), int.Parse(node.Attributes["midiChannel"].Value), data.Length));
